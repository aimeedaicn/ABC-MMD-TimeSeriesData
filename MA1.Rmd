...
# MA(1) model with known parameters
...
nobservation <- 200
ma1_model    <- init_ma1_model(D = nobservation)
rprior       <- ma1_model$rprior
dprior       <- ma1_model$dprior
simulate     <- ma1_model$simulate

method_names <- c("MMD", "Delay reconstruction", "True posterior")

# true parameters
theta_star <- c(0.6, 0.2)  # (theta, log(sigma))
seed_vec   <- c(77)
...
quad_logdet_K <- function(theta, y) {
  a <- 1 + theta^2
  b <- theta
  n <- length(y)
  d <- numeric(n)         
  ...
  z <- numeric(n)
  z[1] <- y[1]
  for (i in 2:n) z[i] <- y[i] - l[i - 1] * z[i - 1]
  ...
  x <- numeric(n)
  x[n] <- w[n]
  for (i in (n - 1):1) x[i] <- w[i] - l[i] * x[i + 1]

  Q <- sum(y * x)                 
  logdetK <- sum(log(d))        
  list(Q = Q, logdetK = logdetK)
}

n_grid   <- 500
theta_g  <- seq(-0.99, 0.99, length.out = n_grid)
logsig_g <- seq(-2, 2, length.out = n_grid)
dtheta   <- theta_g[2]  - theta_g[1]
dlogsig  <- logsig_g[2] - logsig_g[1]

Q_vec       <- numeric(n_grid)
logdetK_vec <- numeric(n_grid)
for (i in seq_along(theta_g)) {
  out <- quad_logdet_K(theta_g[i], y_obs)
  Q_vec[i]       <- out$Q
  logdetK_vec[i] <- out$logdetK
}

n <- length(y_obs)
const_ll <- n * log(2 * pi)

logpost_mat <- matrix(NA_real_, nrow = n_grid, ncol = n_grid)
for (i in seq_along(theta_g)) {
  th  <- theta_g[i]
  base_theta <- -0.5 * (const_ll + logdetK_vec[i]) + log(0.5)

  ls  <- logsig_g
  loglik_vec <- base_theta - 0.5 * (2 * n * ls + Q_vec[i] * exp(-2 * ls))
  logprior_lsg <- dnorm(ls, mean = 0, sd = 1, log = TRUE)

  logpost_mat[i, ] <- loglik_vec + logprior_lsg
}

post_mat <- exp(logpost_mat - max(logpost_mat, na.rm = TRUE))
post_mat <- post_mat / (sum(post_mat, na.rm = TRUE) * dtheta * dlogsig)

density_theta <- rowSums(post_mat, na.rm = TRUE) * dlogsig
density_lsg   <- colSums(post_mat, na.rm = TRUE) * dtheta

df_theta <- tibble(value = theta_g, density = density_theta,
                   param = "theta", method = method_names[3])
df_lsg   <- tibble(value = logsig_g, density = density_lsg,
                   param = "logsigma", method = method_names[3])
df_long  <- bind_rows(df_theta, df_lsg)

write.csv(df_theta, file = file.path(result_dir, "posterior_marginal_theta1.csv"),
          row.names = FALSE)
write.csv(df_lsg,   file = file.path(result_dir, "posterior_marginal_theta2.csv"),
          row.names = FALSE)

my_colours <- c(`True posterior` = brewer.pal(3, "Accent")[1])

plot_marginal <- function(df_sub, true_val, xlab_expr) {
  ggplot(df_sub, aes(x = value, y = density, colour = method, fill = method)) +
...
p_theta <- plot_marginal(df_long |> filter(param == "theta"),
                         theta_star[1], expression(theta))
...
resultsprefix  <- "results/MA1/"
plotprefix     <- "plots/MA1/"

set.seed(77)

theta_star     <- list(theta = c(0.6, 0.2))  
...
true_theta   <- c(theta1 = 0.6, theta2 = 0.2)
...
true_theta <- c(theta = 0.6, logsigma = 0.2)  
...
sabc_to_dataframe <- function(res, colnames_guess = c("theta1","theta2")) {
  as_step_df <- function(M, step_id) {
    ...
    if (ncol(df) >= 2) {
      ...
      df <- df %>% transmute(theta    = .data[[colnames_guess[1]]],
                             logsigma = .data[[colnames_guess[2]]],
                             step     = step_id)
    } else {
      stop("Samples matrix has <2 columns; cannot map to (theta, logsigma).")
    }
    df
  }
...
step_gradient <- scale_colour_gradient2(low = muted("blue"), mid = "yellow",
                                        high = muted("red"),
                                        midpoint = NA,
                                        name = "SMC step")

# MMD result
load("results/MA1/mmd.RData")
...
theta_t <- 0.6
logsigma_t <- 0.2
C <- (1 + theta_t^2) * exp(2*logsigma_t)

theta_seq <- seq(-1, 1, length.out = 500)
ridge_df <- data.frame(
  theta = theta_seq,
  logsigma = 0.5 * (log(C) - log(1 + theta_seq^2))
)


g_left <- ggplot(df_mmd, aes(x = theta, y = logsigma, colour = step)) +
  geom_point(alpha = 0.5, size = 1) +
  scale_colour_gradient2(low = muted("blue"), mid = "yellow",
                         high = muted("red"), midpoint = mid_mmd,
                         name = "SMC step") +
  geom_vline(xintercept = true_theta["theta"], linetype = 2) +
  geom_hline(yintercept = true_theta["logsigma"], linetype = 2) +
  labs(x = expression(theta), y = expression(log(sigma))) +
  geom_line(data = ridge_df, aes(x = theta, y = logsigma),
...
ggsave("plots/MA1/mmd.pdf", g_left, width = 6, height = 5)
ggsave("plots/MA1/mmd_delay.pdf", g_right, width = 6, height = 5)
